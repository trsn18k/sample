 ...
 *
 * During battle, standing pictures are automatically displayed at these timings:
 *   ・Battle start (Fight / Escape selection)
 *   ・Command selection
 *   ・When taking damage
 *   ・When evading
 *   ・When attacking, defending, or using skills
 *   ・When counterattacking or reflecting magic
 *   ・When using items
 *   ・When winning a battle
 *
 * You can define multiple standing pictures with state, switch, and variable
 * conditions:
 *   ・Switch 1 ON + Poison state picture
 *   ・Variable 1 >= 10 + Poison state picture
 *   ・Switch 1 ON picture
 *   ・Poison state picture
 *   ・Normal standing picture with no conditions (required at minimum)
 *
 * Switching by remaining HP%:
 *   First, create a list with "Remaining HP%" = "100".
 *   Copy it, then set "Remaining HP%" = "50" for another list.
 *   When HP drops below half, the "50" list picture will be shown.
 *   You can define multiple pictures for different HP percentages.
 *
 * Image priority order:
 *   1. Matches state ID, switch ID, and variable conditions
 *   2. Matches state ID and switch ID
 *   3. Matches state ID and variable conditions
 *   4. Matches state ID only
 *   5. Matches switch ID and variable conditions
 *   6. Matches switch ID only
 *   7. Matches variable conditions only
 *   8. No conditions (state, switch, and variable not set)
 *   (Among these, the lowest HP% will take priority)
 *
 * To flip an image:
 *   Set X scale to "-100".
 *   (If origin is upper-left, X position shifts left by image width)
 *
 * Counterattack / Magic reflection:
 *   For counterattacks, the standing picture of the attack skill is shown.
 *   For magic reflection, the standing picture of the reflected skill is shown.
 *
 * Plugin command:
 *   Standing Picture ON/OFF: Toggles all standing picture display.
 *
 * Terms of use:
 ...
 * @desc Toggles all standing picture display ON/OFF.
 *
 * @arg enabled
 ...
 * @text On Command
 * @desc Define standing pictures shown during command selection.
 * Multiple pictures can be set per state, switch, or HP%.
 * @default []
 ...
 * @text On Damage
 * @desc Define standing pictures shown when taking damage.
 * Multiple pictures can be set per state, switch, or HP%.
 * @default []
 ...
 * @text On Evasion
 * @desc Define standing pictures shown when evading.
 * Multiple pictures can be set per state, switch, or HP%.
 * @default []
 ...
 * @param sbWinPictures
 * @text On Victory
 * @desc Define standing pictures shown when winning a battle.
 * Multiple pictures can be set per state, switch, or HP%.
 * @default []
 ...
 * @text Attack/Defend/Skill/Counter
 * @desc Define standing pictures shown when attacking, using skills,
 * or using items. Multiple pictures can be set per state, switch, or HP%.
 * @default []
 ...
 * @text On Item Use
 * @desc Define standing pictures shown when using items.
 * Multiple pictures can be set per state, switch, or HP%.
 * @default []
 ...
 * @desc Show standing picture when counterattacking.
 * The picture of the attack skill will be shown.
 * @default true
 * @type boolean
 * @parent counterSettings
 *
 * @param showMagicReflection
 * @text Show on Magic Reflect
 * @desc Show standing picture when reflecting magic.
 * The picture of the reflected skill will be shown.
 * @default true
 * @type boolean
 * @parent counterSettings
 *
 * @param startActorType
 * @text Battle Start Actor
 * @desc Select actor shown at battle start (Fight/Escape).
 * Command picture will be displayed.
 * @type select
 * @default none
 * @option Do Not Show
 * @value none
 ...
 * @param winActorType
 * @text Victory Actor
 * @desc Select actor shown at battle victory.
 * @type select
 * @default lastActor
 * @option Do Not Show
 * @value none
 * @option Last Acting Actor
 * @value lastActor
 * @option First Actor
 * @value firstActor
 * @option Random
 ...
 * @desc Hide standing pictures during enemy selection.
 * @default true
 * @type boolean
 *
 * @param hiddenActorWindow
 * @text Hide on Actor Select
 * @desc Hide standing pictures during target actor selection.
 * @default false
 * @type boolean
 *
 * @param deathBeforeStates
 * @text Death Pre-State Check
 * @desc At death, judge with pre-death state.
 * Turn OFF if using dedicated death pictures.
 * @default false
 * @type boolean
 */
 ...
 * @param memo
 * @text Memo
 * @desc You can write a memo for easier checking in the list view.
 * This has no effect on functionality.
 * @type string
 *
 * @param actorId
 * @text Actor ID
 * @desc Select the actor ID to define standing picture.
 * @type actor
 *
 * @param stateId
 * @text State ID
 * @desc Use when changing picture in a specific state.
 * For normal state, leave blank.
 * @type state
 *
 * @param switchId
 * @text Switch ID
 * @desc Use when changing picture with switch ON.
 * For normal state, leave blank.
 * @type switch
 *
 * @param variableCase
 * @text Variable Cond.
 * @desc Use when changing picture based on variable condition.
 * @default
 * @type struct<variableCase>
 *
 * @param hpPercentage
 * @text HP %
 * @desc Use when changing picture by remaining HP%.
 * For normal, set to 100%.
 * @default 100
 * @min 0
 * @max 100
 * @type number
 ...
 * @param imageName
 * @text Image File
 * @desc Select the image file to display as standing picture.
 * @dir img/pictures
 * @type file
 * @require 1
 *
 ...
 * @param x
 * @text X Pos
 * @desc X display position of standing picture.
 * @default 464
 * @min -9999
 * @max 9999
 * @type number
 ...
 * @param y
 * @text Y Pos
 * @desc Y display position of standing picture.
 * @default 96
 * @min -9999
 * @max 9999
 * @type number
 ...
 * @param motion
 * @text Motion
 * @desc Select the motion animation.
 * @default floatrightfast
 * @type select
 * @option None
 * @value none
 * @option Float In Right (Cmd)
 * @value floatrightfast
 * @option Float In Left (Cmd)
 * @value floatleftfast
 * @option Nod
 * @value yes
 * @option Jump
 ...
 * @option Shake (Damage)
 * @value damage
 * @option Float In Right (Win)
 * @value floatright
 * @option Float In Left (Win)
 * @value floatleft
 * @option Slide Left (Atk)
 * @value stepleft
 * @option Slide Right (Atk)
 * @value stepright
 * @option Head Down (Def)
 * @value headdown
 */
 ...
 * @param itemId
 * @text Skill ID
 * @desc Standing picture will show when this skill is used.
 * If none, shown for all skills including Attack/Defend.
 * @type skill
 *
 * @param stateId
 * @text State ID
 ...
 * @param itemId
 * @text Item ID
 * @desc Standing picture will show when this item is used.
 * If none, shown for all items.
 * @type item
 *
 * @param stateId
 * @text State ID
 ...
 * @param id
 * @text Variable ID
 * @desc Variable ID used in the condition.
 * @type variable
 *
 * @param type
 * @text Compare Type
 * @desc Comparison type with variable ID.
 * @default equal
 * @type select
 * @option Equal
 * @value equal
